...
if (!empty($r[nisn]))
  ...
  {
  mysql_query("INSERT INTO users(username,nama_santri,password) 
...
elseif ($menu=='user' AND $act=='batal')
  {
  mysql_query("DELETE FROM users WHERE user_logi='$_GET[username]'");
  header('location:index.php?menu='.$menu);
  }
...
if (!empty($r['kode_kriteria']))
  ...
  if($data<=100){
    ...
    echo '<script language="javascript">alert("Bobot yang anda masukkan melebihi batas.!"); document.location="index.php?menu=kriteria"</script>';
  }
  }
}
// UPDATE KRITERIA
